...
/**
 * Returns true when the global `--quiet` (`-q`) flag is set. JSON-mode
 * output is independent — `printData` continues to emit JSON to stdout
 * even when `--quiet` is on, since JSON is data not chatter.
 */
export function isQuietMode(command: Command): boolean {
	const globalOpts = command.optsWithGlobals() as { quiet?: boolean };
	return globalOpts.quiet === true;
}
...
function printKeyValue(data: Record<string, unknown>): void {
	...
	for (const key of keys) {
		...
		if (Array.isArray(val)) {
			console.log(
				`  ${chalk.bold(key.padEnd(maxLen))}  ${chalk.dim(`[${val.length} items]`)}`,
			);
